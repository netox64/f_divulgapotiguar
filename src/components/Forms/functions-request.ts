import { IFormAnuncio, IFormImovel, IFormInputCategoria, IFormInputPlano } from "@/components/Forms/types-forms";
import { Anuncio, Categoria, Feedback, Imovel, Notificacao, Pagamento, Plano, Usuario } from "./types-models";
...
export async function getServerSideAnunciosForUsuarioProps(id: string): Promise<Anuncio[] | MessageError> {
    try {
...
export async function getServerSideImoveisForUsuarioProps(id: string): Promise<Imovel[] | MessageError> {
    try {
        const response = await fetch(URL_FRONTEND + `/usuarios/${id}/imoveis`, { method: 'GET' });
        if (!response.ok) {
            return new MessageError(response.statusText);
        }
        const contentType = response.headers.get("Content-Type");
...
export async function getServerSidePlanosForUsuarioProps(id: string): Promise<Plano[] | MessageError> {
    try {
        const response = await fetch(URL_FRONTEND + `/usuarios/${id}/planos`, { method: 'GET' });
        if (!response.ok) {
            return new MessageError(response.statusText);
        }
        const contentType = response.headers.get("Content-Type");
...
export async function postImovel(data: IFormImovel): Promise<Imovel | MessageError> {
    try {
        const response = await fetch(URL_FRONTEND + "/imoveis", {
            method: 'POST',
...
export async function postAnuncio(data: IFormAnuncio): Promise<Anuncio | MessageError> {
    try {
        const response = await fetch(URL_FRONTEND + "/anuncios", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
export async function postCategoria(data: IFormInputCategoria): Promise<Categoria | MessageError> {
    try {
...
export async function postPlano(data: IFormInputPlano): Promise<Plano | MessageError> {
    try {
        const response = await fetch(URL_FRONTEND + "/planos", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
...
//Get By ID
export async function getPlanoById(planoId: number): Promise<Plano | MessageError> {
    try {
...
export async function getAnuncioById(anuncioId: number): Promise<Plano | MessageError> {
    try {
...
// send email
export interface Response { statusCode: number; message: string; }
export async function sendEmailServerSideProps(email: string): Promise<Response> {
    try {
        ...
        if (response.ok) {
            return { statusCode: 200, message: "email enviado com sucesso." } as Response;
        } else if (response.status === 404) {
            return { statusCode: 404, message: "esse email não esta cadastrado no sistema." } as Response;
        }
        return { statusCode: 400, message: "formato de email incooreto." } as Response;
    } catch (error) {
        return { statusCode: 500, message: "o servidor não esta responedo." } as Response;
    }
...
export async function sendRedemServerSideProps(data: IFormInputRedem): Promise<Response> {
    try {
        ...
        if (response.ok) {
            return { statusCode: 200, message: "senha redefinida com sucesso." } as Response;
        }

        return { statusCode: 404, message: "usuario não encontrado, ou token ivalido." } as Response;

    } catch (error) { // Erro de rede pode ocorrer nenhum status é retornado
        return { statusCode: 500, message: "o servidor deve estar fumando e não conseguiu respondeu." } as Response;
    }

}

// register user
type FormRegister = { username: string; email: string; phone: string; cpf: string; password: string; }
export interface IFormInput { name: string; email: string; telefone: string; cpf: string; password: string; repassword: string; }
export async function registerUsuarioServerSideProps(data: IFormInput): Promise<Response> {
    try {
        const response = await fetch(URL_BACKEND + "/auth/register", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ username: data.name, email: data.email, phone: data.telefone, cpf: data.cpf, password: data.password } as FormRegister)
        });

        if (response.status === 201) {
            return { statusCode: 201, message: "registrado com sucesso." } as Response;
        }
        return { statusCode: 400, message: "Ese email ja existe na aplicação, prossiga para redefinição de senha.." } as Response;
    } catch (error) { // Erro de rede pode ocorrer nenhum status é retornado
        return { statusCode: 500, message: "o servidor deve estar fumando e não conseguiu respondeu." } as Response;
    }
...
export interface qrcodeData { username: string; cpf: string; valorOriginal: string };
export async function reqQrCode(data: qrcodeData): Promise<string | MessageError> {
