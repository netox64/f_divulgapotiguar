...
export const FormAnunucio: React.FC<IFormAnunucioProps> = ({ imovelId, planoId, imagem, categoriasIds }) => {
    ...
    const arrayinputField = [{ name: "title", label: "titulo" }, { name: "descricao", label: "descrição" }, { name: "imagemPropaganda", label: "imagem propaganda" },
    { name: "tipoPagamento", label: "tipo de pagamento aceito pelo proprietário" }, { name: "preco", label: "preço cobrado pelo proprietário" },
    //Só durate desenvolvimento, depois lembrar colocar hidden e tals
    { name: "imovelId", label: "id do imovel" }, { name: "planoId", label: "id do plano" }, { name: "usuarioId", label: "id do usuario" }] as inputField[];
